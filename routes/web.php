...
Route::middleware('guest')->group(function () {
    ...
    Route::post('/register', [OrganizationController::class, 'storeRegister'])->name('register.submit');
...
Route::middleware('portal.ready')->group(function () {
    ...
    Route::middleware('voter.guest')->group(function () {
        ...
        Route::post('/', [VoterAuthController::class, 'login'])->middleware('authorize.login')->name('login.submit');
        ...
        // Email verification OTP page (after registration)
        Route::get('/verify-email', [VoterAuthController::class, 'showVerifyEmail'])->name('verify-email');
        Route::post('/verify-email', [VoterAuthController::class, 'verifyEmailOtp'])->name('verify-email.submit');
        // 2FA page (during login)
        Route::get('/2fa', [VoterAuthController::class, 'show2fa'])->name('2fa');
        Route::post('/2fa', [VoterAuthController::class, 'verify2fa'])->name('2fa.submit'); 
        // Resend OTP (used for both verification and 2fa)
        Route::post('/resend-otp', [VoterAuthController::class, 'resendOtp'])->name('resend-otp');
        ...
        Route::get('/reset/password/{token}', [VoterAuthController::class, 'resetView'])
                ->name('password.reset')->middleware('token.check');
    ...
    Route::middleware('voter.auth')->group(function () {
        ...
        Route::post('/cast', [BallotController::class, 'cast'])->name('cast');
...
Route::middleware(['auth'])->prefix('admin')->name('admin.')->group(function () {
    ...
    // Election Settings
    Route::get('/elections/{election}/settings', [ElectionController::class, 'settings'])->name('elections.settings');
    Route::put('/elections/{election}/settings', [ElectionController::class, 'updateSettings'])->name('elections.settings.update');
    // Election Resources
    Route::prefix('elections/{election}')->name('elections.')->group(function () {
        ...
        Route::get('/voters/import-logs/log',     [VoterController::class, 'importLogs'])->name('voters.import-logs.log');
        ...
        // File preview for voter_data file fields (used on the voter show page)
